...
@section('page-title', __('messages.edit_template'))
...
<div class="page-header" style="margin-bottom: 30px;">
    <div class="header-content">
        <div style="display: flex; align-items: center; gap: 16px;">
            ...
            <div>
                <h2 class="text-glow" style="font-size: 28px; font-weight: 800; letter-spacing: -0.5px; margin: 0;">{{ __('messages.edit_template') }}</h2>
                <p style="color: var(--text-secondary); font-size: 14px; margin-top: 4px;">{{ __('messages.modify_sequence_desc') }}</p>
            </div>
        </div>
    </div>
</div>

<form action="{{ route('task-templates.update', $template) }}" method="POST" id="templateForm">
    @csrf
    @method('PUT')
    <div style="display: grid; grid-template-columns: 350px 1fr; gap: 30px; align-items: start;">
        ...
        <div class="glass-card" style="position: sticky; top: 100px; padding: 28px;">
            ...
            <div class="form-group" style="margin-bottom: 24px;">
                ...
                <input type="text" name="name" class="form-control" value="{{ $template->name }}" required placeholder="{{ __('messages.template_name_placeholder') }}" style="background: rgba(0,0,0,0.2); border-color: var(--glass-border);">
            </div>
            ...
            <div class="form-group" style="margin-bottom: 30px;">
                ...
                <textarea name="description" class="form-control" rows="4" placeholder="{{ __('messages.template_desc_placeholder') }}" style="background: rgba(0,0,0,0.2); border-color: var(--glass-border);">{{ $template->description }}</textarea>
            </div>
        ...
        <div class="glass-card" style="padding: 32px;">
            ...
            <div id="tasksContainer" style="position: relative; padding-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}: 40px;">
                ...
                @foreach($template->items as $index => $item)
                    <div class="task-row" data-index="{{ $index }}" style="position: relative; margin-bottom: 30px;">
                        <!-- Step Dot -->
                        ...
                        <div style="background: rgba(255,255,255,0.03); border: 1px solid var(--glass-border); border-radius: 20px; padding: 24px;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    @php
                                        ...
                                        $currentIcon = $iconMap[$item->type] ?? 'fa-tasks';
                                    @endphp
                                    <div class="type-icon" style="width: 40px; height: 40px; background: rgba(14, 165, 233, 0.1); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 18px; color: var(--brand-cyan);">
                                        <i class="{{ strpos($currentIcon, 'fab') !== false ? $currentIcon : 'fas ' . $currentIcon }}"></i>
                                    </div>
                            ...
                            <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="items[{{ $index }}][title]" value="{{ $item->title }}" class="form-control" required style="background: rgba(0,0,0,0.2);">
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="items[{{ $index }}][type]" class="form-control type-selector" style="background: rgba(0,0,0,0.2);">
                                        <option value="follow_up" {{ $item->type == 'follow_up' ? 'selected' : '' }}>{{ __('messages.follow_up') }}</option>
                                        <option value="call" {{ $item->type == 'call' ? 'selected' : '' }}>{{ __('messages.call') }}</option>
                                        <option value="whatsapp" {{ $item->type == 'whatsapp' ? 'selected' : '' }}>{{ __('messages.whatsapp') }}</option>
                                        <option value="email" {{ $item->type == 'email' ? 'selected' : '' }}>{{ __('messages.email') }}</option>
                                        <option value="meeting" {{ $item->type == 'meeting' ? 'selected' : '' }}>{{ __('messages.meeting') }}</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="items[{{ $index }}][priority]" class="form-control" style="background: rgba(0,0,0,0.2);">
                                        <option value="low" {{ $item->priority == 'low' ? 'selected' : '' }}>{{ __('messages.low') }}</option>
                                        <option value="medium" {{ $item->priority == 'medium' ? 'selected' : '' }}>{{ __('messages.medium') }}</option>
                                        <option value="high" {{ $item->priority == 'high' ? 'selected' : '' }}>{{ __('messages.high') }}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 20px;">
                                <div class="form-group">
                                    ...
                                    <div style="display: flex; align-items: center; background: rgba(0,0,0,0.2); border-radius: 12px; border: 1px solid var(--glass-border); overflow: hidden;">
                                        ...
                                        <input type="number" name="items[{{ $index }}][delay_days]" value="{{ $item->delay_days }}" class="form-control" min="0" style="border: none; background: transparent; text-align: center; padding: 10px 0;">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="items[{{ $index }}][description]" class="form-control" rows="1" style="background: rgba(0,0,0,0.2);">{{ $item->description }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let rowCount = {{ $template->items->count() }};
